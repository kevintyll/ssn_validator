...
== Note:  
The SSA has since changed how they assign SSNs, so there is no way to determine the validity SSN assignment.
The death master file functionality is still valid.
...
* Can also pass optional validation date, so you can see if the SSN would have been on the DMF file as of that date. Defaults to today.  Pass a date or a string that can be evalutated by Date.parse
  Be aware that the as of date is the date you updated your local DMF table, not when the SSN was actually added to the DMF.  If you update
  your local table monthly, then the 2 should be equivelant.
